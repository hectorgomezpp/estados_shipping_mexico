var mkrapel_mx_settings = (function($, window, document) {
	'use strict';
	...
	var OPTION_ROW_HTML  = '<tr>';
		...
		OPTION_ROW_HTML += '<td class="action-cell"><a href="javascript:void(0)" onclick="mkrapelmxAddNewOptionRow(this)" class="btn btn-blue" title="Add new option">+</a></td>';
		OPTION_ROW_HTML += '<td class="action-cell"><a href="javascript:void(0)" onclick="mkrapelmxRemoveOptionRow(this)" class="btn btn-red" title="Remove option">x</a></td>';
		OPTION_ROW_HTML += '<td class="action-cell sort ui-sortable-handle"></td>';
		OPTION_ROW_HTML += '</tr>';

	$(function() {
		$( "#mkrapel_mx_new_field_form_pp" ).dialog({
		  	modal: true,
			width: 600,
			resizable: false,
			autoOpen: false,
			buttons: [{
				text: "Save",
				click: function() {
					var form = $("#mkrapel_mx_new_field_form");
		...
		$( "#mkrapel_mx_edit_field_form_pp" ).dialog({
		  	modal: true,
			width: 600,
			resizable: false,
			autoOpen: false,
			buttons: [{
				text: "Save",
				click: function() {
					var form = $("#mkrapel_mx_edit_field_form");
					var valid = validate_field_form(form);
					if(valid){ 
						prepare_field_form(form);
						form.submit(); 
		...
		$('select.mkrapel-mx-enhanced-multi-select').select2({
			minimumResultsForSearch: 10,
			allowClear : true,
			placeholder: $(this).data('placeholder')
		}).addClass('enhanced');

		$('#mkrapel_mx_checkout_fields tbody').sortable({
			items:'tr',
			cursor:'move',
			axis:'y',
			handle: 'td.sort',
		...
		$("#mkrapel_mx_checkout_fields tbody").on("sortstart", function( event, ui ){
			ui.item.css('background-color','#f6f6f6');										
		});
		$("#mkrapel_mx_checkout_fields tbody").on("sortstop", function( event, ui ){
	...
	function openNewFieldForm(sname){
		...
		set_property_field_value(form, 'text', 'class', 'form-row-wide', 0);
	  	...
	  	$("#mkrapel_mx_new_field_form_pp").dialog("open");
	}
	...
	function openEditFieldForm(elm, rowId){
		...
		populate_field_form(row, form, props);
	  	...
	  	$("#mkrapel_mx_edit_field_form_pp").dialog("open");
	}
	...
	*------------------------------------*/
	...
	function populate_options_list(form, optionsJson){
			...
			try{
				...
				if(optionsList){
					jQuery.each(optionsList, function() {
						...
						html += '<td class="action-cell"><a href="javascript:void(0)" onclick="mkrapelmxAddNewOptionRow(this)" class="btn btn-blue" title="Add new option">+</a></td>';
						html += '<td class="action-cell"><a href="javascript:void(0)" onclick="mkrapelmxRemoveOptionRow(this)" class="btn btn-red" title="Remove option">x</a></td>';
		...
		var optionsTable = form.find(".mkrapel-mx-option-list tbody");
		if(optionsHtml){
	...
	*------------------------------------*/
	...
	function prepare_field_order_indexes() {
		$('#mkrapel_mx_checkout_fields tbody tr').each(function(index, el){
			$('input.f_order', el).val( parseInt( $(el).index('#mkrapel_mx_checkout_fields tbody tr') ) );
		});
	};
	
	function selectAllCheckoutFields(elm){
		var checkAll = $(elm).prop('checked');
		$('#mkrapel_mx_checkout_fields tbody input:checkbox[name=select_field]').prop('checked', checkAll);
	}

	function removeSelectedFields(){
		$('#mkrapel_mx_checkout_fields tbody tr').removeClass('thpladmin-strikeout');
		$('#mkrapel_mx_checkout_fields tbody input:checkbox[name=select_field]:checked').each(function () {
...
function mkrapelmxOpenNewFieldForm(tabName){
	mkrapel_mx_settings.openNewFieldForm(tabName);		
}

function mkrapelmxOpenEditFieldForm(elm, rowId){
	mkrapel_mx_settings.openEditFieldForm(elm, rowId);		
}
	
function mkrapelmxRemoveSelectedFields(){
	mkrapel_mx_settings.removeSelectedFields();
}

function mkrapelmxEnableSelectedFields(){
	mkrapel_mx_settings.enableDisableSelectedFields(1);
}

function mkrapelmxDisableSelectedFields(){
	mkrapel_mx_settings.enableDisableSelectedFields(0);
}

function mkrapelmxFieldTypeChangeListner(elm){	
	mkrapel_mx_settings.fieldTypeChangeListner(elm);
}
	
function mkrapelmxSelectAllCheckoutFields(elm){
	mkrapel_mx_settings.selectAllCheckoutFields(elm);
}

function mkrapelmxAddNewOptionRow(elm){
	mkrapel_mx_settings.addNewOptionRow(elm);
}
function mkrapelmxRemoveOptionRow(elm){
	mkrapel_mx_settings.removeOptionRow(elm);
}