jQuery(document).ready(function($) {
	$('select.mkrapel-mx-enhanced-select').select2({
	...
	$( document.body ).bind( 'country_to_state_changing', function( event, country, wrapper ) {
		if(mkrapel_mx_public_var.is_override_required){
	  	...
	  	if(locale_fields){
	  		$.each( locale_fields, function( key, value ) {
				...
				$.each( fids, function( index, fid ) {
					...
					if(field.hasClass('mkrapel-mx-required')){
						field_is_required( field, true );
					}else if(field.hasClass('mkrapel-mx-optional')){
