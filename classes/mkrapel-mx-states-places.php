	...
	*/
	public function wc_form_field_city($field, $key, $args, $value ) {
		...
		if ( is_array( $places ) ) {
			...
			$field .= '<option value="">'. __( 'Seleciona una ciudad&hellip;', 'woocommerce' ) .'</option>';
			if ( $current_sc ) {
	...
	*/
	...
	public function load_scripts() {
		if ( is_cart() || is_checkout() || is_wc_endpoint_url( 'edit-address' ) ) {
			$city_select_path = self::get_plugin_url() . 'assets/js/mkrapel-mx-place-select.js';
			...
			wp_localize_script( 'wc-city-select', 'wc_city_select_params', array(
				'cities' => $places,
				'i18n_select_city_text' => esc_attr__( 'Selecciona una ciudad&hellip;', 'woocommerce' )
			) );
		}
	}
