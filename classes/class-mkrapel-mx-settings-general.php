...
if(!class_exists('MKRAPEL_MX_Settings_General')):

class MKRAPEL_MX_Settings_General {
	...
	public function render_checkout_fields_heading_row(){
		?>
		<th class="sort"></th>
		<th class="check-column"><input type="checkbox" style="margin:0px 4px -1px -1px;" onclick="mkrapelmxSelectAllCheckoutFields(this)"/></th>
	...
	public function render_actions_row($section){
		?>
        <th colspan="6">
            <button type="button" class="button button-primary" onclick="mkrapelmxOpenNewFieldForm('<?php echo $section; ?>')" style="border-radius: 20px;">+ <?php _e( 'Agregar Campo', 'wc-ciudades-y-regiones-de-chile' ); ?></button>
            <button type="button" class="button" onclick="mkrapelmxRemoveSelectedFields()" style="border-radius: 20px;"><?php _e('Remover', 'wc-ciudades-y-regiones-de-chile'); ?></button>
            <button type="button" class="button" onclick="mkrapelmxEnableSelectedFields()" style="border-radius: 20px;"><?php _e('Activar', 'wc-ciudades-y-regiones-de-chile'); ?></button>
            <button type="button" class="button" onclick="mkrapelmxDisableSelectedFields()" style="border-radius: 20px;"><?php _e('Desactivar', 'wc-ciudades-y-regiones-de-chile'); ?></button>
        </th>
        <th colspan="4">
	...
	public function output_content() {
		...
		$fields = MKRAPEL_MX_Utils::get_fields($section);	
	
		?>            
        <div class="wrap woocommerce"><div class="icon32 icon32-attributes" id="icon-woocommerce"><br /></div>
		<form method="post" id="mkrapel_mx_checkout_fields_form" action="">
        	<table id="mkrapel_mx_checkout_fields" class="mkrapel_mx_fields_table" cellspacing="0">
				<thead>
				...
				<tbody class="ui-sortable">
					...
					foreach( $fields as $name => $field ) :
						...
						if($type === 'select' || $type === 'radio'){
							...
							$options_json = MKRAPEL_MX_Utils::prepare_options_json($options);
						}
					?>
						<tr class="row_<?php echo $i; echo $enabled ? '' : ' mkrapel-mx-disabled' ?>">
	                        ...
	                        <td class="td_label"><?php MKRAPEL_MX_Utils::et($label); ?></td>
	                        <td class="td_placeholder"><?php MKRAPEL_MX_Utils::et($placeholder); ?></td>
	                        ...
	                        <td class="td_edit action">
	                            ...
	                            onclick="mkrapelmxOpenEditFieldForm(this, <?php echo $i; ?>)"><?php _e('Editar', 'wc-ciudades-y-regiones-de-chile'); ?></button>
	                        </td>
	                	</tr>
	                <?php 
	                	$i++; 
	...
	private function save_or_update_field($section, $action) {
		try {
			...
			if($name){
				if($action === 'new'){
					$priority = MKRAPEL_MX_Utils::prepare_field_priority($fields, false, true);
					$field['custom'] = 1;
			...
			$result = MKRAPEL_MX_Utils::update_fields($section, $fields);
		...
		try {
			...
			for($i = 0; $i <= $max; $i++) {
				...
				if(isset($fields[$name])){
					...
					$priority = MKRAPEL_MX_Utils::prepare_field_priority($fields, $order, false);
			...
			$fields = MKRAPEL_MX_Utils::sort_fields($fields);
			$result = MKRAPEL_MX_Utils::update_fields($section, $fields);
	...
	private function prepare_field_from_posted_data($posted){
		...
		if($type === 'select' || $type === 'radio'){
			...
			$options_arr = MKRAPEL_MX_Utils::prepare_options_array($options_json);
	...
	public function get_field_form_props(){
		...
		return array(
			'type' 		  => array('type'=>'select', 'name'=>'type', 'label'=>'Tipo', 'required'=>1, 'options'=>$field_types, 
								'onchange'=>'mkrapelmxFieldTypeChangeListner(this)'),
	...
	private function output_add_field_form_pp(){
		?>
        <div id="mkrapel_mx_new_field_form_pp" title="Nuevo Campo del Formulario de Pago" class="mkrapel-mx-popup-wrapper">
	...
	private function output_edit_field_form_pp(){		
		?>
        <div id="mkrapel_mx_edit_field_form_pp" title="Editar Campo del Formulario de Pago" class="mkrapel-mx-popup-wrapper">
	...
	private function output_popup_form_fields($form_type){
		...
		?>
		<form method="post" id="mkrapel_mx_<?php echo $form_type ?>_field_form" action="">
	...
	private function render_form_field_element_select($props){
		...
		if(isset($props['multiple']) && $props['multiple']){
			...
			$field_attr .= ' class="mkrapel-mx-enhanced-multi-select"';
	...
	private function render_field_form_fragment_options(){
		...
		<tr class="row-options">
			...
			<td>
				<table width="100%" border="0" cellpadding="0" cellspacing="0" class="mkrapel-mx-option-list mkrapel-mx-dynamic-row-table"><tbody>
					<tr>
						...
						<td class="action-cell"><a href="javascript:void(0)" onclick="mkrapelmxAddNewOptionRow(this)" class="btn btn-blue" title="Agregar Nueva Opción">+</a></td>
						<td class="action-cell"><a href="javascript:void(0)" onclick="mkrapelmxRemoveOptionRow(this)" class="btn btn-red" title="Eliminar Opción">x</a></td>
	...
	public function order_data_after_order_details($order){
		$fields = MKRAPEL_MX_Utils::get_fields('additional');
		$this->display_fields_in_admin_order($order, $fields, '<p>&nbsp;</p>');
	}
	public function order_data_after_billing_address($order){
		$fields = MKRAPEL_MX_Utils::get_fields('billing');
		$this->display_fields_in_admin_order($order, $fields, '');
	}
	public function order_data_after_shipping_address($order){
		$fields = MKRAPEL_MX_Utils::get_fields('shipping');
		$this->display_fields_in_admin_order($order, $fields, '');
	}
	public function display_fields_in_admin_order($order, $fields, $prefix_html = ''){
		if(is_array($fields)){
			$html = '';
			$order_id = MKRAPEL_MX_Utils::get_order_id($order);
		
			foreach($fields as $name => $field){
				if(MKRAPEL_MX_Utils::is_active_custom_field($field) && isset($field['show_in_order']) && $field['show_in_order']){
					$value = get_post_meta( $order_id, $name, true );
					if(!empty($value)){
						$value = MKRAPEL_MX_Utils::get_option_text($field, $value);
						$label = isset($field['label']) && $field['label'] ? MKRAPEL_MX_Utils::t($field['label']) : $name;
	...
	public function output_tabs(){
		...
		echo '<h2 class="mkrapel-mx-tabs nav-tab-wrapper woo-nav-tab-wrapper">';
	...
	public function output_sections() {
		...
		echo '<ul class="mkrapel-mx-sections">';
