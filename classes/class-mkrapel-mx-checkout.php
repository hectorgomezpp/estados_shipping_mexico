...
if(!class_exists('MKRAPEL_MX_Checkout')) :

class MKRAPEL_MX_Checkout {
	public function __construct() {
		
	}
	public function enqueue_styles_and_scripts() {
		if(is_checkout()){
			$in_footer = apply_filters( 'mkrapel_mx_enqueue_script_in_footer', true );
			$deps = array('jquery', 'select2');

			wp_register_script('mkrapel-mx-checkout-script', MKRAPEL_MX_ASSETS_URL.'js/mkrapel-mx-checkout.js', $deps, MKRAPEL_MX_VERSION, $in_footer);
			wp_enqueue_script('mkrapel-mx-checkout-script');
			wp_enqueue_style('mkrapel-mx-checkout-style', MKRAPEL_MX_ASSETS_URL . 'css/mkrapel-mx-public.css', MKRAPEL_MX_VERSION);

			$wcfd_var = array(
				'is_override_required' => $this->is_override_required_prop(),
			);
			wp_localize_script('mkrapel-mx-checkout-script', 'mkrapel_mx_public_var', $wcfd_var);
		}
	}
	public function define_public_hooks(){
		$hp_billing_fields  = apply_filters('mkrapel_mx_billing_fields_priority', 1000);
		$hp_shipping_fields = apply_filters('mkrapel_mx_shipping_fields_priority', 1000);
		$hp_checkout_fields = apply_filters('mkrapel_mx_checkout_fields_priority', 1000);
	...
	private  function get_locale_override_value($key, $settings=false, $default=false){
		...
		if($settings){
			$value = MKRAPEL_MX_Utils::get_setting_value($settings, $key);
		}else{
			$value = MKRAPEL_MX_Utils::get_settings($key);
		}
	...
	public function is_override_label($settings=false){
		...
		return apply_filters('mkrapel_mx_address_field_override_label', $override_label);
	}
	public function is_override_placeholder($settings=false){
		...
		return apply_filters('mkrapel_mx_address_field_override_placeholder', $override_ph);
	}
	public function is_override_class($settings=false){
		...
		return apply_filters('mkrapel_mx_address_field_override_class', $override_class);
	}
	public function is_override_priority($settings=false){
		...
		return apply_filters('mkrapel_mx_address_field_override_priority', $override_priority);
	}
	public function is_override_required_prop($settings=false){
		...
		return apply_filters('mkrapel_mx_address_field_override_required', $override_required);
	}
	public function prepare_country_locale($fields) {
		if(is_array($fields)){
			$settings = MKRAPEL_MX_Utils::get_advanced_settings();
	...
	public function prepare_checkout_fields($fields, $original_fields) {
		if(is_array($fields) && !empty($fields)) {
			...
			foreach($fields as $name => $field) {
				if(MKRAPEL_MX_Utils::is_enabled($field)) {
					$new_field = false;
					$allow_override = apply_filters('mkrapel_mx_allow_default_field_override_'.$name, false);
					...
					if($original_fields && isset($original_fields[$name]) && !$allow_override){
						...
						if($override_required_prop){
							if($required){
								$class[] = 'mkrapel-mx-required';
							}else{
								$class[] = 'mkrapel-mx-optional';
							}
						}
					...
					$new_field['class'][] = 'mkrapel-mx-field-wrapper';
					$new_field['class'][] = 'mkrapel-mx-field-'.$type;
					
					if($type === 'select' || $type === 'radio'){
						if(isset($new_field['options'])){
							$options_arr = MKRAPEL_MX_Utils::prepare_field_options($new_field['options']);
							$options = array();
					...
					if($type === 'select' && apply_filters('mkrapel_mx_enable_select2_for_select_fields', true)){
						$new_field['input_class'][] = 'mkrapel-mx-enhanced-select';
					}
					
					if(isset($new_field['label'])){
						$new_field['label'] = MKRAPEL_MX_Utils::t($new_field['label']);
					}

					if(isset($new_field['placeholder'])){
						$new_field['placeholder'] = MKRAPEL_MX_Utils::t($new_field['placeholder']);
					}
	...
	*************************************/
	public function checkout_fields_validation($posted, $errors){
		...
		foreach($checkout_fields as $fieldset_key => $fieldset){
			...
			foreach($fieldset as $key => $field) {
				if(isset($posted[$key]) && !MKRAPEL_MX_Utils::is_blank($posted[$key])){
		...
		if($key){
			...
			if($value && is_array($validators) && !empty($validators)){					
				foreach($validators as $vname){
					$err_msg = '';
					$flabel = isset($field['label']) ? MKRAPEL_MX_Utils::t($field['label']) : $key;

					if($vname === 'number'){
						if(!is_numeric($value)){
							$err_msg = '<strong>'. $flabel .'</strong> '. MKRAPEL_MX_Utils::t('is not a valid number.');	
						}
					}
	...
	public function add_validation_error($msg, $errors=false){
		...
		}else if(MKRAPEL_MX_Utils::woo_version_check('2.3.0')){
	...
	public function checkout_update_order_meta($order_id, $posted){
		...
		foreach($types as $type){
			...
			$fields = MKRAPEL_MX_Utils::get_fields($type);
			
			foreach($fields as $name => $field){
				if(MKRAPEL_MX_Utils::is_active_custom_field($field) && isset($posted[$name])){
	...
	*****************************************/
	public function display_custom_fields_in_emails($ofields, $sent_to_admin, $order){
		$custom_fields = array();
		$fields = MKRAPEL_MX_Utils::get_checkout_fields();
		...
		foreach( $fields as $key => $field ) {
			if(isset($field['show_in_email']) && $field['show_in_email']){
				...
				if($value){
					...
					$custom_field = array();
					$custom_field['label'] = MKRAPEL_MX_Utils::t($label);
	...
	public function order_details_after_customer_details($order){
		$order_id = MKRAPEL_MX_Utils::get_order_id($order);
		$fields = MKRAPEL_MX_Utils::get_checkout_fields($order);
		...
		if(is_array($fields) && !empty($fields)){
			...
			foreach($fields as $key => $field){			
				if(MKRAPEL_MX_Utils::is_active_custom_field($field) && isset($field['show_in_order']) && $field['show_in_order']){
					...
					if($value){
						$label = isset($field['label']) && $field['label'] ? MKRAPEL_MX_Utils::t($field['label']) : $key;
						...
						if(is_account_page()){
							if(apply_filters( 'mkrapel_mx_view_order_customer_details_table_view', true )){
								$fields_html .= '<tr><th>'. $label .':</th><td>'. $value .'</td></tr>';
							}else{
								$fields_html .= '<br/><dt>'. $label .':</dt><dd>'. $value .'</dd>';
							}
						}else{
							if(apply_filters( 'mkrapel_mx_thankyou_customer_details_table_view', true )){
								$fields_html .= '<tr><th>'. $label .':</th><td>'. $value .'</td></tr>';
							}else{
								$fields_html .= '<br/><dt>'. $label .':</dt><dd>'. $value .'</dd>';
							}
			...
			if($fields_html){
				do_action( 'mkrapel_mx_order_details_before_custom_fields_table', $order ); 
				?>
				...
				<?php
				do_action( 'mkrapel_mx_order_details_after_custom_fields_table', $order ); 
			}
		}
	}
}
